...
define([
...
], function ($, _, utils, moment, ko, confirm, alert) {
    'use strict';
    ...
    var isCisCountry = ko.observable(false);
    var isCourierPartnerCountry = ko.observable(false);
    var lastCountry = ko.observable(false);
    var kycFormAccepted = ko.observable(false);
    var taxFormAccepted = ko.observable(false);
    var isCourierPartnerCountry_tax = ko.observable(false);
    var isCourierPartnerCountry_all = ko.observable(false);
    ...
    return _.mapObject({
        ...
        'validate-phone-number': [
            function(value) {
             return utils.isEmptyNoTrim(value) || /^[\+]?[-\s]?[(]?[0-9-+() ]{5}[)]?[-]?[-\s]?[0-9-+() ]{0,15}$/im.test(value);
		//return utils.isEmptyNoTrim(value) || /^[-]?[\+]?[(]?[0-9+() ]{3}[)]?[-\s\.]?[0-9+() ]{2}[-\s\.]?[0-9+() ]{0,8}$/im.test(value);
		 //  return utils.isEmptyNoTrim(value) || /^[\+]?[(]?[0-9+() ]{3}[)]?[-\s\.]?[0-9+() ]{3}[-\s\.]?[0-9+() ]{4,8}$/im.test(value);
            },
        ...
        'validate-country-section-1': [
            function(value) {
                var countryArray = ['AM', 'AZ', 'BY', 'GE', 'KZ', 'KG', 'MD', 'RU', 'TJ', 'TM', 'UA', 'UZ'];
                var selector = 'select[name="country_id"]';
                var ESmessage = '';
                if ($.inArray(value, countryArray) >= 0) {
                    if(!isCisCountry() || lastCountry() != value){
                       /* if(value === 'ES'){
                        ...
                        confirm({
                            ...
                            actions: {
                                confirm: function () {
                                    isCisCountry = ko.observable(true);
                                    $(selector).css('border-color', '#d1d1d1');
                                    $(selector).nextAll('.mage-error').css('display', 'none');
                                },
                                cancel: function () {
                                    isCisCountry = ko.observable(false);
                                },
                                always: function () {
                                },
                ...
                if ($.inArray(value, countryArray) >= 0) {
                    if(!isCourierPartnerCountry() || lastCountry() != value){
                        confirm({
                            ...
                            actions: {
                                confirm: function () {
                                    isCourierPartnerCountry = ko.observable(true);
                                    $(selector).css('border-color', '#d1d1d1');
                                    $(selector).nextAll('.mage-error').css('display', 'none');
                                },
                                cancel: function () {
                                    isCourierPartnerCountry = ko.observable(false);
                                },
                                always: function () {
                                },
                                clickableOverlay: false
                    ...
                    return isCourierPartnerCountry();
                }
                return true;
            },
            $.mage.__('Please confirm to contact you by Dilmah or your local authorities or Courier Company')
        ],
        'validate-country-section-3': [
            function (value) {
                var countryArray = ['IN'];
                if ($.inArray(value, countryArray) >= 0 && !kycFormAccepted()) {
                    alert({
                        title: 'Important!',
                    ...
                    kycFormAccepted = ko.observable(true);
                    return true;
                }
                kycFormAccepted = ko.observable(false);
                return true;
            },
            ''
        ],
        
        'validate-country-section-4': [
            function(value) {
				var countryArray = ['TR', 'CL'];
                var selector = 'select[name="country_id"]';
                if ($.inArray(value, countryArray) >= 0) {
                    if(!isCourierPartnerCountry_tax() || lastCountry() != value){
                        confirm({
                            modalClass: 'confirm country-selection',
                            title: 'Important!',
							content: 'Please Kindly Note The selecfted country Required Tax ID for Online Tea Purchase, Please submit to the relevant authorities before clearing your Tea. Sometimes you need to get help of Clearance Agency to Clear your Tea. which may acquire additional charge which may not include in courier cost.',
                            actions: {
                                confirm: function () {
                                    isCourierPartnerCountry_tax = ko.observable(true);
                                    $(selector).css('border-color', '#d1d1d1');
                                    $(selector).nextAll('.mage-error').css('display', 'none');
                                },
                                cancel: function () {
                                    isCourierPartnerCountry_tax = ko.observable(false);
                                },
                                always: function () {
                                },
                                clickableOverlay: false
                    ...
                    return isCourierPartnerCountry_tax();
                }
                return true;
            },
            $.mage.__('Please confirm to contact you by Dilmah or your local authorities or Courier Company')
        ],
		'validate-country-section-5': [
            function(value) {
				var countryArray = ['GB','CN','IL','AU','US','SI','SE','CH','TH','PL','IE','HU','IT','DE','DK','CZ','FR','CA','BE'];
                var selector = 'select[name="country_id"]';
               var storeId     = window.checkoutConfig.quoteData.store_id;
               if(storeId == '1' || storeId == '1') {

		 if ($.inArray(value, countryArray) >= 0) {
                    if(!isCourierPartnerCountry_all() || lastCountry() != value){
                        confirm({
                            modalClass: 'confirm country-selection',
                            title: 'Important!',
                            content: 'The selected country has mandatory Phytosanitary procedure, which may acquire additional charge which may not include in courier cost.',
                            actions: {
                                confirm: function () {
                                    isCourierPartnerCountry_all = ko.observable(true);
                                    $(selector).css('border-color', '#d1d1d1');
                                    $(selector).nextAll('.mage-error').css('display', 'none');
                                },
                                cancel: function () {
                                    isCourierPartnerCountry_all = ko.observable(false);
                                },
                                always: function () {
                                },
                                clickableOverlay: false
                    ...
                    return isCourierPartnerCountry_all();
                }
               }
                return true;
            },
   ...
   if ($.inArray(value, countryArray) >= 0) {
                    if(!isCourierPartnerCountry_all() || lastCountry() != value){
                        confirm({
                            modalClass: 'confirm country-selection',
                            title: 'Important!',
                            content: 'Please note broker and the following costs are currently applied: 1. Duties and Taxes (approx 32% which is calculated from the value + shipping costs)  2. Airport Delivery order (agency fees)   3. Clearance fee',
                            actions: {
                                confirm: function () {
                                    isCourierPartnerCountry_all = ko.observable(true);
                                    $(selector).css('border-color', '#d1d1d1');
   ...
   if ($.inArray(value, countryArray) >= 0) {
                    if(!isCourierPartnerCountry_all() || lastCountry() != value){
                        confirm({
                            modalClass: 'confirm country-selection',
                            title: 'Important!',
                            content: 'Please note that, as per destination customs regulations “Food, non-perishables” are restricted to export by courier mode.',
                            actions: {
                                confirm: function () {
                                    isCourierPartnerCountry_all = ko.observable(true);
                                    $(selector).css('border-color', '#d1d1d1');
       ...
       /* 
        'validate-country-section-4': [
            function (value) {
                var countryArray = ['TR', 'CL'];
                if ($.inArray(value, countryArray) >= 0 && !taxFormAccepted() ) {
                    alert({
                        title: 'Important!',
                        content: 'Please Kindly Note The selected country Required Tax ID for Online Tea Purchase, Please submit to the relevant authorities before clearing your Tea. Sometimes you need to get help of Clearance Agency to Clear your Tea. which may acquire additional charge which may not include in courier cost.',
                        actions: {
                            always: function () {
                            }
                        }
                    });
					taxFormAccepted = ko.observable(true);
                    return true;
                }
				taxFormAccepted = ko.observable(false);
                return true;
            },
            ''
        ],         
       ...
       /* 'validate-country-section-5': [
            function (value) {
				var storeId     = window.checkoutConfig.quoteData.store_id;
				if(storeId == '1' || storeId == '1') {
                var countryArray = ['GB','CN','IL','AU','US','SI','SE','CH','TH','PL','IE','HU','IT','GR','DE','DK','CZ','FR','CA','BE'];
                if ($.inArray(value, countryArray) >= 0 && !kycFormAccepted()) {
                    alert({
                        title: 'Important!',
        i                content: 'The selected country has mandatory Phytosanitary procedure, which may acquire additional charge which may not include in courier cost.',
                        actions: {
                            always: function () {
                            }
                        }
