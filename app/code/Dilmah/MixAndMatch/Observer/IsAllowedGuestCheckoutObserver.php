...
namespace Dilmah\MixAndMatch\Observer;
...
class IsAllowedGuestCheckoutObserver implements ObserverInterface
{
    ...
    /**
     * Check is allowed guest checkout if quote contain downloadable product(s) || if quote contain Mix And Match Product(s)
     *
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        foreach ($quote->getAllItems() as $item) {
            if (
                ...
                ||
                (($product = $item->getProduct())
                    && $product->getMixAndMatchPromotion()
                )
