<?php
/**
 * Generate order report daily, execute by cronjob
 */
...
class Report extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
		...
		$dir       = "common-header/ordersreport";
		if (is_dir($dir) === false) {
			mkdir($dir);
			chmod($dir,0777);
		}
		$filename 	= "C_WebOrders_" . $fname_date . ".csv";
        $outputFile = BP . "/common-header/ordersreport/" .$filename;
		...
		$expredate 	= "C_WebOrders_" . $expredate;
		...
		foreach($files as $file){
			...
			$d_file = $d_file[0];
			if($file < $expredate ){
				$DeleteFile = BP . "/common-header/ordersreport/" .$file;
		...
		echo $toDate; 
		die;		*/
		$orderCollection = $this->_orderCollectionFactory->create()->addAttributeToFilter('created_at', array('from' => $fromDate, 'to' => $toDate));
        if ($status != 'all') {
	...
	$add_existing_plan 	= $this->getExistingPlan($postpaidCollection,$quoteid);
	 ...
	 $row = array();
                $row = [
                    ...
                    '', //nationality is blank on 1802 NID
