...
class Website extends AbstractImportValidator implements RowValidatorInterface
{
    ...
    public function isValid($value)
    {
        $this->_clearMessages();
        $valid = true;
        if (isset($value[BasePriceImport::COL_SPECIAL_PRICE]) && !empty($value[BasePriceImport::COL_SPECIAL_PRICE])) {
            $valid *= $this->isWebsiteValid($value, BasePriceImport::COL_SPECIAL_PRICE_WEBSITE);
        }
        if (!$valid) {
    ...
    {
        return BasePriceImport::VALUE_ALL_WEBSITES . ' ['.$this->websiteModel->getBaseCurrency()->getCurrencyCode().']';
    }
}
