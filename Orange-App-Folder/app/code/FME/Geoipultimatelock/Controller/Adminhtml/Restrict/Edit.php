...
{ 
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('FME_Geoipultimatelock::geoipultimatelock')
                ...
                ->addBreadcrumb(__('Manage Restrict IPs'), __('Manage Restrict IPs'));
        return $resultPage;
    }
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('geoipultimatelock_restrict_data', $model);
        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Restrict Info') : __('New Restrict Info'),
            $id ? __('Edit Restrict Info') : __('New Restrict Info')
        );
        ...
        $resultPage->getConfig()
                ->getTitle()
                ->prepend($model->getId() ? $model->getTitle() : __('Geo IP Ultimate Lock - Restrict IP(s)'));
