 ...
 * Time: 10:37
 */
namespace Magenest\AbandonedCartReminder\Block\Adminhtml\Rule\Add\Tab\Email\Media\Gallery;
    ...
    protected function _prepareLayout()
    {
        ...
        $this->getUploader()->getConfig()->setUrl(
          //  $this->_urlBuilder->addSessionParam()->getUrl('demo/media/upload')
            $this->_urlBuilder->addSessionParam()->getUrl('abandonedcartreminder/rule/upload')
        )->setFileField(
            'image'
        )->setFilters(
            [
    ...
    public function getImagesJson()
    {
      //$ruleId =
        ...
        if (is_object($model)) {
            ...
            $value = unserialize($imagesString);
            if (is_array($value) &&
                array_key_exists('images', $value) &&
                is_array($value['images']) &&
                count($value['images'])
