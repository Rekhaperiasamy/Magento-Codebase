...
namespace Orange\Mediaimport\Model\Import;
...
class Product extends \Magento\CatalogImportExport\Model\Import\Product {
     ...
     public function deleteProductsForReplacement() {
          $this->setParameters(array_merge(
                          $this->getParameters(), ['behavior' => Import::BEHAVIOR_DELETE]
          ));
               ...
               if ($idsToDelete) {
                    ...
                    try {
                         $this->objectRelationProcessor->delete(
                                 $this->transactionManager, $this->_connection, $productEntityTable, $this->_connection->quoteInto('entity_id IN (?)', $idsToDelete), ['entity_id' => $idsToDelete]
                         );
     ...
     protected function _replaceProducts() {
          ...
          $this->_validatedRows = null;
          $this->setParameters(array_merge(
                          $this->getParameters(), ['behavior' => Import::BEHAVIOR_APPEND]
          ));
          $this->_saveProductsData();
     ...
     protected function _saveLinks() {
          ...
          foreach ($this->_linkNameToId as $linkName => $linkId) {
               $select = $this->_connection->select()->from(
                               $resource->getTable('catalog_product_link_attribute'), ['id' => 'product_link_attribute_id']
                       )->where(
          ...
          while ($bunch = $this->_dataSourceModel->getNextBunch()) {
               ...
               foreach ($bunch as $rowNum => $rowData) {
                    ...
                    $productLinkKeys = [];
                    $select = $this->_connection->select()->from(
                                    $resource->getTable('catalog_product_link'), ['id' => 'link_id', 'linked_id' => 'linked_product_id', 'link_type_id' => 'link_type_id']
                            )->where(
                    ...
                    foreach ($this->_linkNameToId as $linkName => $linkId) {
                         ...
                         if (isset($rowData[$linkName . 'sku'])) {
                              ...
                              $linkPositions = !empty($rowData[$linkName . 'position']) ? explode($this->getMultipleValueSeparator(), $rowData[$linkName . 'position']) : [];
               ...
               if (Import::BEHAVIOR_APPEND != $this->getBehavior() && $productIds) {
                    $this->_connection->delete(
                            $mainTable, $this->_connection->quoteInto('product_id IN (?)', array_unique($productIds))
                    );
               }
               if ($linkRows) {
               ...
               if ($positionRows) {
                    ...
                    $this->_connection->insertOnDuplicate(
                            $resource->getAttributeTypeTable('int'), $positionRows, ['value']
                    );
               }
          }
          return $this;
     ...
     protected function _saveProductCategories(array $categoriesData) {
          ...
          if ($categoriesData) {
               ...
               if (Import::BEHAVIOR_APPEND != $this->getBehavior()) {
                    $this->_connection->delete(
                            $tableName, $this->_connection->quoteInto('product_id IN (?)', $delProductId)
                    );
               }
               if ($categoriesIn) {
     ...
     public function saveProductEntity(array $entityRowsIn, array $entityRowsUp) {
          ...
          if ($entityRowsIn) {
               ...
               $select = $this->_connection->select()->from(
                               $entityTable, $this->getNewSkuFieldsForSelect()
                       )->where(
     ...
     protected function getExistingImages($bunch) {
          ...
          $select = $this->_connection->select()->from(
                          ['mg' => $this->mediaGalleryTableName], ['value' => 'mg.value']
                  )->joinInner(
                          ['mgvte' => $this->mediaGalleryEntityToValueTableName], '(mg.value_id = mgvte.value_id)', [$this->getProductEntityLinkField() => 'mgvte.' . $this->getProductEntityLinkField()]
                  )->joinInner(
                          ['pe' => $this->productEntityTableName], "(mgvte.{$this->getProductEntityLinkField()} = pe.{$this->getProductEntityLinkField()})", ['sku' => 'pe.sku']
                  )->where(
                  'pe.sku IN (?)', $productSKUs
          );
     ...
     protected function _saveProducts() {
          ...
          while ($bunch = $this->_dataSourceModel->getNextBunch()) {
               ...
               foreach ($bunch as $rowNum => $rowData) {
                    if (isset($rowData['sku']) && $rowData['sku'] && isset($rowData['name']) && $rowData['name']) {
                         $metaProductTitle = '';
                         $metaTitle = '';
                         $id = '';
                         if (isset($rowData['meta_product_title'])) {
                              $metaProductTitle = $rowData['meta_product_title'];
                         }
                         if ($rowData['store_view_code'] != 'nl') {
                              if ($metaProductTitle != '') {
                                   $metaTitle = $rowData['name'] . ' - ' . $metaProductTitle . ' | Orange';
                              } else {
                                   $metaTitle = $rowData['name'] . ' ' . $metaProductTitle . ' | Orange';
                              }
                         }
                         if (isset($rowData['meta_title']) && $rowData['meta_title']) {
                              
                         } else {
                              $rowData['meta_title'] = substr(htmlentities(strip_tags($metaTitle), ENT_COMPAT, 'UTF-8'), 0, 159);
                         }
                         if (isset($rowData['meta_description']) && $rowData['meta_description']) {
                              
                         } else {
                              $metaDescription = '';
                              if ($rowData['store_view_code'] != 'nl') {
                                   if ($rowData['store_view_code'] == '') {
                                        $metaDescription = 'Koop je ' . $rowData['name'] . ' met aan ' . $rowData['price'];
                                   } else {
                                        $metaDescription = 'Acheter votre ' . $rowData['name'] . ' avec à ' . $rowData['price'];
                                   }
                                   if (isset($rowData['marketing_description']) && $rowData['product_type'] == 'simple') {
                                        $metaDescription = substr(htmlentities(strip_tags($rowData['marketing_description']), ENT_COMPAT, 'UTF-8'), 0, 159);
                                   }
                              }
                              $rowData['meta_description'] = $metaDescription;
                         }
                    }
                    // $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                    // $objectManager->get('Psr\Log\LoggerInterface')->addDebug('Store:'.$rowData['store_view_code']);

                    if (!$this->validateRow($rowData, $rowNum)) {
                         continue;
                    }
                    if (isset($rowData[self::COL_NAME])) {
                         if (isset($rowData['product_type']) && isset($rowData['attribute_set_code']) && $rowData['product_type'] == 'virtual' && $rowData['attribute_set_code'] == 'Postpaid') {
                              $rowData['url_key'] = $this->productUrl->formatUrlKey($rowData[self::COL_NAME]) . "-orange";
                         } else {
                              $rowData['url_key'] = $this->productUrl->formatUrlKey($rowData[self::COL_NAME]);
                         }
                    }
                    ...
                    if (isset($this->_oldSku[$rowSku])) {
						...
						if(isset($rowData['price'])){
							$rowPrice = $rowData['price'];							
						}
						if(isset($rowData['store_view_code'])){
							$storeCodeView = $rowData['store_view_code'];
						}
						if((isset($rowData['attribute_set_code']) && isset($rowData['attribute_set_code'] )) && ($rowData['attribute_set_code'] == 'handset' || $rowData['attribute_set_code'] == 'Accessories')){
							$catV = 1;
						}						
						
                         // existing row
						 //send value to catalog version module
                         if(isset($rowData['product_type']) && $rowData['product_type'] == 'simple'  && $catV != 0
						 ){
							$objectManager = \Magento\Framework\App\ObjectManager::getInstance();					
							$catalogVersion = $objectManager->create('Orange\Catalogversion\Model\Catalogversion')->catalogver($this->_oldSku[$rowSku],$rowPrice,$storeCodeView);
						}
                         $entityRowsUp[] = [
                         ...
                         foreach ($columnImages as $position => $columnImage) {
                              if (!isset($uploadedImages[$columnImage])) {
                                   ...
                                   if ($uploadedFile) {
                                        ...
                                        $this->addRowError(
                                                ValidatorInterface::ERROR_MEDIA_URL_NOT_ACCESSIBLE, $rowNum, null, null, ProcessingError::ERROR_LEVEL_NOT_CRITICAL
                                        );
                                   }
                              } else {
                    ...
                    $rowData = $productTypeModel->prepareAttributesWithDefaultValueForSave(
                            $rowData, !isset($this->_oldSku[$rowSku])
                    );
                    ...
                    foreach ($rowData as $attrCode => $attrValue) {
                         ...
                         if ('datetime' == $attribute->getBackendType() && strtotime($attrValue)) {
                              $attrValue = $this->dateTime->gmDate(
                                      'Y-m-d H:i:s', $this->_localeDate->date($attrValue)->getTimestamp()
                              );
                         } elseif ($backModel) {
     ...
     protected function catalogVersion($rowData) {
          ...
          if ($currentproduct->getData('store_id')) {
               $store_id = $currentproduct->getData('store_id');
          } else {
               $store_id = 0;
          }
          ...
          $stockStatus = $currentproduct->getData('is_in_stock');
          $quantity = $currentproduct->getData('qty');
          ...
          foreach ($attributesCollection as $attribute) {
               ...
               if ($currentproduct->getData($attributeCode)) {
                    ...
                    if ($attribute['frontend_input'] == 'select') {
                         ...
                         //echo $valueforOptionID = $currentproduct->getData($attributeCode);
          ...
          $catalogVersion->setStoreId($store_id);
          ...
          $catalogVersion->save();
          echo $currentproduct->getData('sku');
          print_r($prod);
          print_r($finalData);
          exit;
     }
     ...
     protected function processRowCategories($rowData) {
          ...
          if (!empty($categoriesString)) {
               $categoryIds = $this->categoryProcessor->upsertCategories(
                       $categoriesString, $this->getMultipleValueSeparator()
               );
               foreach ($this->categoryProcessor->getFailedCategories() as $error) {
                    $this->errorAggregator->addError(
                            AbstractEntity::ERROR_CODE_CATEGORY_NOT_VALID, ProcessingError::ERROR_LEVEL_NOT_CRITICAL, $rowData['rowNum'], self::COL_CATEGORY, __('Category "%1" has not been created.', $error['category'])
     ...
     protected function _saveProductTierPrices(array $tierPriceData) {
          ...
          if ($tierPriceData) {
               ...
               if (Import::BEHAVIOR_APPEND != $this->getBehavior()) {
                    $this->_connection->delete(
                            $tableName, $this->_connection->quoteInto("{$this->getProductEntityLinkField()} IN (?)", $delProductId)
                    );
               }
               if ($tierPriceIn) {
     ...
     protected function _saveMediaGallery(array $mediaGalleryData) {
          ...
          $stores = $om->get('Magento\Store\Model\StoreRepository')->getList();
          foreach ($mediaGalleryData as $productSku => $mediaGalleryRows) {
               foreach ($mediaGalleryRows as $insertValue) {
                    foreach ($newMediaValues as $value_id => $values) {
                         if ($values['value'] == $insertValue['value']) {
                              $insertValue['value_id'] = $value_id;
                              $insertValue[$this->getProductEntityLinkField()] = array_shift($valueToProductId[$values['value']]);
                    ...
                    if (isset($insertValue['value_id'])) {
                         foreach ($stores as $store) {
                              ...
                              $labelTitle = str_replace($insertValue['product_color'], '', $insertValue['product_name']);
                              if ($storeId == 2) {
                                   $imgLabel = 'Orange Belgique ' . $labelTitle . ' - ' . $insertValue['product_color']; //set Image title
                              } else {
                                   $imgLabel = 'Orange België ' . $labelTitle . ' - ' . $insertValue['product_color']; //set Image title
                              }
                              $valueArr = [
          ...
          try {
               $this->_connection->insertOnDuplicate(
                       $this->mediaGalleryValueTableName, $multiInsertData, ['value_id', 'store_id', $this->getProductEntityLinkField(), 'label', 'position', 'disabled']
               );
               $this->_connection->insertOnDuplicate(
                       $this->mediaGalleryEntityToValueTableName, $dataForSkinnyTable, ['value_id']
               );
          } catch (\Exception $e) {
               $this->_connection->delete(
                       $this->mediaGalleryTableName, $this->_connection->quoteInto('value_id IN (?)', $newMediaValues)
               );
          }
          return $this;
     }
     ...
     protected function _saveStockItem() {
          ...
          while ($bunch = $this->_dataSourceModel->getNextBunch()) {
               ...
               foreach ($bunch as $rowNum => $rowData) {
                    ...
                    $row = array_merge(
                            $this->defaultStockData, array_intersect_key($existStockData, $this->defaultStockData), array_intersect_key($rowData, $this->defaultStockData), $row
                    );
                    ...
                    if ($this->stockConfiguration->isQty(
                         ...
                         if ($this->stockStateProvider->verifyNotification($stockItemDo)) {
                              $row['low_stock_date'] = $this->dateTime->gmDate(
                                      'Y-m-d H:i:s', (new \DateTime())->getTimestamp()
                              );
                         }
                         if ($row['qty'] > 0) {
          ...
          if (isset($this->_oldSku[$sku])) {
               ...
               if (isset($this->_productTypeModels[$this->_oldSku[$sku]['type_id']])) {
                    $this->skuProcessor->addNewSku(
                            $sku, $this->prepareNewSkuData($sku)
                    );
               } else {
     ...
     private function isNeedToValidateUrlKey($rowData) {
          return (!empty($rowData[self::URL_KEY]) || !empty($rowData[self::COL_NAME])) && (empty($rowData[self::COL_VISIBILITY]) || $rowData[self::COL_VISIBILITY] !== (string) Visibility::getOptionArray()[Visibility::VISIBILITY_NOT_VISIBLE]);
     }
     ...
     private function _parseAdditionalAttributes($rowData) {
          ...
          foreach ($attributeNameValuePairs as $attributeNameValuePair) {
               ...
               if ($separatorPosition !== false) {
                    ...
                    $value = substr(
                            $attributeNameValuePair, $separatorPosition + strlen(self::PAIR_NAME_VALUE_SEPARATOR)
                    );
     ...
     private function _setStockUseConfigFieldsValues($rowData) {
          ...
          foreach ($rowData as $key => $value) {
               ...
               if (isset($this->defaultStockData[$key]) && isset($this->defaultStockData[$useConfigName]) && !empty($value) && empty($rowData[$useConfigName])
               ) {
          ...
          foreach ($this->urlKeys as $storeId => $urlKeys) {
               $urlKeyDuplicates = $this->_connection->fetchAssoc(
                       $this->_connection->select()->from(
                                       ['url_rewrite' => $resource->getTable('url_rewrite')], ['request_path', 'store_id']
                               )->joinLeft(
                                       ['cpe' => $resource->getTable('catalog_product_entity')], "cpe.entity_id = url_rewrite.entity_id"
     ...
     protected function getProductUrlSuffix($storeId = null) {
          if (!isset($this->productUrlSuffix[$storeId])) {
               $this->productUrlSuffix[$storeId] = $this->scopeConfig->getValue(
                       \Magento\CatalogUrlRewrite\Model\ProductUrlPathGenerator::XML_PATH_PRODUCT_URL_SUFFIX, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId
               );
          }
