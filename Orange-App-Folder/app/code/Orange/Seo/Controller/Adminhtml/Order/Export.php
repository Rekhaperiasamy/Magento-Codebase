...
namespace Orange\Seo\Controller\Adminhtml\Order;
...
define('NET_SFTP_LOCAL_FILEBI', 1);
...
class Export extends \Magento\Framework\App\Action\Action {
	...
	public function execute() {
        ...
        $orderCollection = $this->_orderCollectionFactory->create()
		     //->addAttributeToFilter('increment_id', '100072120');
	...
	if ($bbox) {
		$bbox = "\t".$bbox."\t";
			...
			foreach($postpaidModelCollections as $postpaidCollection) {
			    if (!$addToExistingPlan) {
					$addToExistingPlan = $this->boxData('is_teneuro_'.$postpaidCollection->getItemId(),$quoteid);
					if ($addToExistingPlan == "yes") {
						$cuExIEWIInvoiceCustNumber = $this->boxData('iew_telephone_'.$postpaidCollection->getItemId(),$quoteid);
						$cuExIEWIInvoiceCustNumber = "\t".$cuExIEWIInvoiceCustNumber."\t";
						$iewContract    = $this->boxData('iew_contract_'.$postpaidCollection->getItemId(), $quoteid);
						if ($iewContract == "0") {
							$cuIEWFirstName         = $this->boxData('iew_first_name_'.$postpaidCollection->getItemId(), $quoteid);
							$cuIEWlastName          = $this->boxData('iew_last_name_'.$postpaidCollection->getItemId(), $quoteid);
							$cuIEWDob                = $this->boxData('iew_dob_'.$postpaidCollection->getItemId(), $quoteid);
						}
					}
				}
			}
...
if($this->boxData('scoringResponse', $quoteid) == 'ACC'){ $scoring_decline = '';}else{ $scoring_decline = $this->boxData('scoringResponse', $quoteid);}
     ...
     $lob = $this->getProductLOB($orderItem);
	 $MobileClientType = $this->mobileclienttype($orderItem->getSku(),$orderItem->getProductType(),$product_model);
	 if($lob == 'IEW' || $MobileClientType =="new IEW client"){
			$showtoaddToExistingPlan = $addToExistingPlan;
			$showtocuIInvoiceCustNumber = $cuExIEWIInvoiceCustNumber;
			$showtocuFirstName = $cuIEWFirstName;
			$showtoculastName = $cuIEWlastName;
			$showtocuDob   = $cuIEWDob;
			$showtoContract = $iewContract;
	 } else {
			$showtoaddToExistingPlan = $this->getExistingPlan($quoteid,$orderItem);
			$showtocuIInvoiceCustNumber = $this->getcustomer_number($quoteid,$showtoaddToExistingPlan,$orderItem);
			$showtocuFirstName = $this->getinvoiceownerfname($orderItem,$quoteid);
			$showtoculastName = $this->getinvoiceownerlname($orderItem,$quoteid);
			$showtocuDob   =  $this->getinvoiceownerdob($orderItem,$quoteid);
			$showtoContract = $this->getinvoiceowner($orderItem,$quoteid);
	 ...
	 $row = array();
                $row = [
                    ...
                    $showtoaddToExistingPlan, //add_to_existing_plan
                    $showtocuIInvoiceCustNumber, //customer_number
					...
					$showtocuFirstName, //invoice_owner_first_name
                    $showtoculastName, //invoice_owner_last_name
                    $showtocuDob, //invoice_owner_date_of_birth
                    $showtoContract, //invoice_owner
                    ...
                    $this->getreduction($orderItem,$showtoaddToExistingPlan), //Reduction on IEW
		...
		/* Push code to remote server */
			...
			//$reportPath = '/var/www/html/common-header/';
/*			
...
*/
    ...
    public function ftpDetails() {
		$scopeConfig = $this->objectManagerInt()->create('Magento\Framework\App\Config\ScopeConfigInterface');
		$Credentials = array();
        $Credentials['Froudreport_Host'] = $scopeConfig->getValue('fraudreport_ftpdetails/fraudreport__configuration/server_detail', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $Credentials['Froudreport_User'] = $scopeConfig->getValue('fraudreport_ftpdetails/fraudreport__configuration/server_user_id', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $Credentials['Froudreport_Password'] = $scopeConfig->getValue('fraudreport_ftpdetails/fraudreport__configuration/server_pwd', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $Credentials['Froudreport_ppk'] = $scopeConfig->getValue('fraudreport_ftpdetails/fraudreport__configuration/server_ppkey', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $Credentials['Froudreport_Path'] = $scopeConfig->getValue('fraudreport_ftpdetails/fraudreport__configuration/server_directorypath', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        return $Credentials;
    }
...
public function getgsm($gsm){	
    ...
    public function deliverMethod($method, $quote_id,$type) {
		...
		if($type == 'prepaid'){
			...
			if ($deleiveryAddressInformation == 3) {
				   $deliveryMethod = $this->boxData('deliveryMethod',$quote_id);
				   if($deliveryMethod == "Pugo") {
						$deleiveryMethod = strtoupper($deliveryMethod);
	...
	public function getcustomer_number($quoteid,$plan,$orderItemm){
     
        $lob = $this->getProductLOB($orderItemm);
			
		if($plan == 'yes' && $lob == 'IEW' )
		{
          $custiewnumber = $this->boxData('iew_telephone_'.$orderItemm->getQuoteItemId(),$quoteid );

			$result_data = "\t".$custiewnumber."\t";
			return $result_data;
	...
	public function getExistingPlan($quoteid,$orderItemmnew){
		$lob = $this->getProductLOB($orderItemmnew);
				
		if($lob == 'IEW'){
			return $this->boxData('is_teneuro_'.$orderItemmnew->getQuoteItemId(), $quoteid);
		}else{
