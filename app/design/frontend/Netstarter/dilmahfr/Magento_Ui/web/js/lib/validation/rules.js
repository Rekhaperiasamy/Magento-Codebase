...
define([
    ...
    return _.mapObject({
        ...
        "validate-email": [
            ...
            $.mage.__('Veuillez renseigner une adresse email valide (ex : johndoe@domain.com).')
        ],
        ...
        'validate-phone-number': [
            function(value) {
                return utils.isEmptyNoTrim(value) || /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/im.test(value);
            },
        ...
        'validate-country-section-1': [
            function(value) {
                var countryArray = ['AM', 'AZ', 'BY', 'GE', 'KZ', 'KG', 'MD', 'RU', 'ES', 'TJ', 'TM', 'UA', 'UZ'];
                var selector = '.country-validation-note';
                if ($.inArray(value, countryArray) >= 0) {
                    $(selector).html('Please contact us at orders@dilmahtea.com before placing an order');
                    $(selector).css('color', 'red');
                    return false;
                }
                // below line is only used in section 1 validation, if it is used in section 2, it might empty the
                // error message when the section 2 ('validate-country-section-2') is fired.
                $(selector).html('');
                return true;
            },
            ''
