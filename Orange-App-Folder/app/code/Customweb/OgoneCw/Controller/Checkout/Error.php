		...
		if (!empty($transactionId)) {
			...
			if ($transaction->getOrder() instanceof \Magento\Sales\Model\Order) {
				if ($transaction->getOrder()->canCancel()) {	
					$incrementId = $transaction->getOrder()->getIncrementId(); //Added for incident 39120247 coupon reactivation
					$transaction->getOrder()->cancel()->save();	
					$this->reactivateCoupon($incrementId); //Added for incident 39120247 coupon reactivation					
				}
			}
		}
