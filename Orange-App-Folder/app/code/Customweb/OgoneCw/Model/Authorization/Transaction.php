...
class Transaction extends \Magento\Framework\Model\AbstractModel implements \Magento\Sales\Model\EntityInterface, TransactionInterface
{
	...
	public function afterSave()
	{
		...
		/*To Decline Unncertain Declined Orders by Ogone*/
		if($this->isUncertaindeclined()) {
		 $this->CancelUncertaindeclined();
		}
	...
	protected function registerReviewAction()
	{
		if ($this->getTransactionObject()->isUncertainTransactionFinallyDeclined()) {
			...
			$this->getOrder()->save();
			$this->uncertainMailsend($this->getOrder());
	...
	public function uncertainMailsend($order){
	    $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
	    $objectManager->get('Magento\Sales\Model\Order\Email\Sender\OrderSender')->send($order);
    }
	public function isUncertaindeclined(){
	$parmeters = $this->getTransactionObject()->getTransactionData();
       if(isset($parmeters['STATUS']) && ($parmeters['STATUS'] == 93)){
          return true;
		  } else {
          return false;
          }
	...
	$parmeters = $this->getRequestParamsUncertain()->getParameters();
       if(isset($parmeters['CARDNO']) && isset($parmeters['PAYID']) && $parmeters['CARDNO'] != "" && $parmeters['PAYID'] != "" ){
          return true;
		  } else {
          return false;
          }
	}
	public function CancelUncertaindeclined(){
	    $objectManager = \Magento\Framework\App\ObjectManager::getInstance();	
		$order = $this->getOrder();
		$orderState = \Magento\Sales\Model\Order::STATE_PENDING_PAYMENT;
		$order->setState($orderState)->setStatus($orderState);
		$order->save();
		$objectManager->create('\Magento\Sales\Api\OrderManagementInterface')->cancel($order->getEntityId());
    }
	public function saveCardpayments(){
		$order = $this->getOrder();
		$parmeters = $this->getRequestParamsUncertain()->getParameters();
		$cardno = $parmeters['CARDNO'];
		$payid  = $parmeters['PAYID'];
	...
	public function getRequestParamsUncertain(){
		return new \Customweb_Core_Http_ContextRequest();
	} 
}