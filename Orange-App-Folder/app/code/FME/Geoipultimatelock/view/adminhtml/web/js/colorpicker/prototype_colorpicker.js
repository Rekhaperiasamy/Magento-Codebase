var colorPicker=Class.create(
    ...
    initialize:function () {
        ...
        this.fields.each(
            ...
            field.observe(
                'change', function (ev) {
                this.change(ev.element());}.bind(this)
            );
        ...
        $$('#'+this.id+' span').each(
            function (span) {
            span.observe('mousedown', this.downIncrement.bind(this));}.bind(this)
        );
        ...
        if ($(this.options.previewElement)) {
            $(this.options.previewElement).setStyle('backgroundColor':'#'+this.HSBToHex(this.color)});
    ...
    setCurrentColor:function (hsb) {
        ...
        if (!this.options.origColor) {
            this.options.origColor = hsb;
        }
    },
    ...
    change:function (el) {
        ...
        if (this.options.updateOnChange) {
            if (this.el.nodeName == 'INPUT') {
                this.el.value = this.HSBToHex(col);
            }

            if ($(this.options.inputElement)) {
                $(this.options.inputElement).value = this.HSBToHex(col);
            }

            if ($(this.options.previewElement)) {
                $(this.options.previewElement).setStyle('backgroundColor':'#'+this.HSBToHex(col)});
        }
    },
    blur:function (ev) {
    ...
    upIncrement:function (ev) {
        ...
        if (ev.element().up().className.indexOf('_hsb')!=-1) {
            this.fillRGBFields(this.color);
        } else {
            this.fillHSBFields(this.color);
        }
    ...
    show:function (ev) {
        ...
        if (this.options.onShow(this)) {
            this.cp.setStyle(display:'block'});
