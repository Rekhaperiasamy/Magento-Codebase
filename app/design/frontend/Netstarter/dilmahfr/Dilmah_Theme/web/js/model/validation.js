...
(function (factory) {
    ...
    var rules = {
        "equalTo": [
            ...
            $.mage.__("Entrez à nouveau la même valeur s'il vous plait.")
        ],
        "max-words": [
        ...
        "validate-customer-password": [
            function (v, elm) {
                ...
                if (result == false) {
                    validator.passwordErrorMessage = $.mage.__("Ce champ doit contenir au moins %1 caractéres.").replace('%1', passwordMinLength) + " " +
                        $.mage.__("Les espaces en début ou en fin de mot de passe ne seront pas pris en compte.");
                    return result;
                }
                if (pass.match(/\d+/)) {
                    counter ++;
                ...
                if (counter < passwordMinCharacterSets) {
                    result = false;
                    validator.passwordErrorMessage = $.mage.__("Merci d utiliser au minimum %1 types de caractéres").replace('%1', passwordMinCharacterSets) + " " +
                        $.mage.__("différents (minuscules, majuscules, chiffres, caractéres spéciaux).");
                }
                return result;
            }, function () {
