 ...
 */
namespace Dilmah\Catalog\Block\Product\View\Options\Type;
...
class Select extends \Magento\Catalog\Block\Product\View\Options\Type\Select
{
    ...
    public function getValuesHtml()
    {
        ...
        $pos = strpos(strtolower($_option->getTitle()), 'engrav');
        $engraveClass = '';
        $engraveClassCommon = '';
        $engraveClassNone = '';
        if ($pos !== false) {
            if ($product->getIsEngravable() == 1) {
                $engraveClass = ' engraving';
                $engraveClassCommon = ' engrave-radio';
                $engraveClassNone = ' engrave-none';
            }
        }
        ...
        if ($_option->getType() == \Magento\Catalog\Model\Product\Option::OPTION_TYPE_RADIO ||
            ...
            foreach ($_option->getValues() as $_value) {
                ...
                $selectHtml .= '<div class="field choice admin__field admin__field-option' . $engraveClass .
                    $require .
                    '">' .
                    '<input type="' .
                    $type .
